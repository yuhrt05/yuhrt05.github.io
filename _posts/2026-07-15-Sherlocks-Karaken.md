...
|---|---|---|---|
| Execution | T1059.007 | Command and Scripting Interpreter: JavaScript/JScript | Initial malicious `.js` file executed through Windows Script Host (`wscript.exe`). |
| Execution | T1059.003 | Command and Scripting Interpreter: Windows Command Shell | The malware dropped and executed batch scripts including `temp_993805.bat` and `dwm.bat`. |
| Execution | T1059.001 | Command and Scripting Interpreter: PowerShell | The batch stage reconstructed and executed an embedded PowerShell payload using `Invoke-Expression`. |
| Defense Evasion | T1027.010 | Obfuscated Files or Information: Command Obfuscation | PowerShell code was fragmented across numerous batch variables and reconstructed at runtime. API and function names were also dynamically reconstructed. |
| Defense Evasion | T1027.013 | Obfuscated Files or Information: Encrypted/Encoded File | Embedded payloads were protected using Base64 encoding and AES-CBC encryption. |
| Defense Evasion | T1140 | Deobfuscate/Decode Files or Information | Payloads were Base64-decoded, AES-CBC decrypted and GZip decompressed before execution. |
| Defense Evasion | T1562.001 | Impair Defenses: Disable or Modify Tools | The malware patched AMSI scanning routines, patched `EtwEventWrite`, and later added a Microsoft Defender exclusion for the C: drive. |
| Defense Evasion | T1620 | Reflective Code Loading | Decrypted .NET assemblies were loaded directly from byte arrays with `Assembly.Load()` and executed through their entry points. |
| Persistence | T1547.001 | Boot or Logon Autostart Execution: Registry Run Keys / Startup Folder | A randomly named batch file such as `5c74.bat` was created inside the user's Startup folder to maintain persistence. |
| Command and Control | T1071.001 | Application Layer Protocol: Web Protocols | The final-stage payload communicated with its C2 server using HTTP requests and randomized HTTP User-Agent strings. |
